...
const TasksView = ({
  ...
  const scrollLeft = () => {
    ...
    const scrollAmount = (typeof window !== 'undefined' && window.innerWidth < 768) ? 1 : 3; // Scroll by 1 on mobile, 3 on desktop
  ...
  const getTaskStatus = (task) => {
    const submission = submissions.find(s => {
      // Handle both cases where taskId might be an object or string
      const taskIdToCompare = s.taskId?._id || s.taskId;
      return taskIdToCompare === task._id;
    });
    
    console.log('Submission for task', task._id, ':', submission);

    // Check if task is completed (has been graded with a score)
    if (submission && (submission.status === 'graded' || (submission.score !== undefined && submission.score !== null))) {
      return 'completed';
    }
    
    // Check if task is submitted but not yet graded
    if (submission && (submission.status === 'submitted' || submission.status === 'pending')) {
      return 'under_review';
    }
